...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    const t = await getTranslations({ locale, namespace: "SourceLabGrownDiamondsPage" });

    return {
        title: t("metadata.title"),
        description: t("metadata.description"),
        alternates: {
            canonical: `https://www.uniglodiamonds.com/${locale === "en" ? "" : locale + "/"}source-lab-grown-diamonds-for-jewelers`,
        },
    };
}

const SourceLabGrownDiamondsPage = async ({ params }: Props) => {
    const resolvedParams = await Promise.resolve(params);
    const locale = resolvedParams.locale;
    const t = await getTranslations({ locale, namespace: "SourceLabGrownDiamondsPage" });
