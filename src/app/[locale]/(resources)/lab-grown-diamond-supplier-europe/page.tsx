...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    const t = await getTranslations({ locale, namespace: "LabGrownDiamondSupplierPage" });

    return {
        title: t("metadata.title"),
        description: t("metadata.description"),
        alternates: {
            canonical: `https://www.uniglodiamonds.com/${locale === "en" ? "" : locale + "/"}lab-grown-diamond-supplier-jewelers`,
        },
    };
}

const LabGrownDiamondSupplierPage = async ({ params }: Props) => {
    const resolvedParams = await Promise.resolve(params);
    const locale = resolvedParams.locale;
    const t = await getTranslations({ locale, namespace: "LabGrownDiamondSupplierPage" });
