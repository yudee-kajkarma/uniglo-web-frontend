...
export async function generateMetadata({
    ...
    if (locale === "de") {
        ...
        description =
            "Durchsuchen Sie unsere Melee-Diamantenpakete - natürlich und im Labor gezüchtet, sortiert nach Siebgröße, Farbe und Reinheit.";
    } else if (locale === "nl") {
        title =
            "Melee Diamanten | Gecertificeerde Losse Percelen | Uniglo Diamonds";
        description =
            "Blader door onze melee diamantpercelen - natuurlijk en laboratoriumgekweekt, gesorteerd op zeefgrootte, kleur en helderheid.";
    } else if (locale === "fr") {
        title =
            "Diamants de Mêlée | Parcelles en Vrac Certifiées | Uniglo Diamonds";
        description =
            "Parcourez nos lots de diamants de mêlée - naturels et de laboratoire, triés par taille de tamis, couleur et pureté.";
    } else if (locale === "it") {
        title =
            "Diamanti da Mischia | Lotti Sfusi Certificati | Uniglo Diamonds";
        description =
            "Sfoglia i nostri lotti di diamanti da mischia - naturali e coltivati in laboratorio, ordinati per dimensione del setaccio, colore e purezza.";
    } else if (locale === "es") {
        title =
            "Diamantes Cuerpo a Cuerpo | Parcelas Sueltas Certificadas | Uniglo Diamonds";
        description =
            "Explore nuestros lotes de diamantes cuerpo a cuerpo, naturales y cultivados en laboratorio, clasificados por tamaño de tamiz, color y claridad.";
    }
...
export default async function MeleeDiamondsPage({
    params,
    searchParams,
}: PageProps) {
    const { locale } = await params;
    ...
    return (
        <main className="max-w-6xl mx-auto px-4 py-28 font-lora">
            ...
            <p className="mt-2 text-xs text-gray-500">
                {totalRecords.toLocaleString()} {t.stones} · {t.page}{" "}
